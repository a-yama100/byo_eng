...
function AdviceComponent() {
  ...
  const handleSubmit = async () => {
    const response = await fetch('http://localhost:3000/advice', {
        ...
        body: JSON.stringify({ text: userInput, language: language }),
    });
  ...
  return (
    <div>
      ...
      <select className="form-control mb-3" value={language} onChange={handleLanguageChange}>
        <option value="en">英語で返信</option>
        <option value="ja">日本語で返信</option>
      </select>
